...
# ファイル読み込みのユーティリティ関数
def read_file_safely(filename, encodings=['utf-8', 'utf-16', 'utf-16-le', 'shift_jis', 'euc-jp']):
    """
    複数のエンコーディングを試してファイルを安全に読み込みます。

    Args:
        filename: ファイルパス
        encodings: 試すエンコーディングのリスト（優先順）
    ...
    Raises:
        ...
        UnicodeError: どのエンコーディングでも解読できない場合
    """
    ...
    # バイナリモードで読み込んでエンコーディングを検出する最後の試み
    try:
        with open(filename, 'rb') as f:
            ...
            # それ以外はエラーを発生
            raise UnicodeError(f"ファイル {filename} のエンコーディングを検出できませんでした")
    except Exception as e:
        raise IOError(f"ファイル {filename} の読み込みに失敗しました: {str(e)}")
